...
public class ExpenseDashboardService {
	...
	public List<ExpenseSummaryDetail> expenseSummaryByBranchId(Integer branchId, Date startDate, Date startWeek,
		...
		return jdbc.query(
				"select exps.branch_id, " + "max(branch.name) as branch_name,max(branch.name_kh) as branch_name_kh, "
						...
						+ "and exps.branch_id = :branchId " + "group by exps.branch_id",
				mapSqlParameterSource, (rs, rowNum) -> new ExpenseSummaryDetail(rs.getInt("branch_id"),
						rs.getString("branch_name"), rs.getString("branch_name_kh"), rs.getDouble("monthly_sale_amount"),rs.getDouble("weekly_sale_amount"),rs.getDouble("daily_sale_amount")));
	}
	public List<ExpenseTypeSummaryDetail> expenseTypeSummaryByBranchId(Integer branchId, Date startDate,
			Date endDate) {
		if (userProfile.getProfile().getBranch().getId() != branchId) {
			throw new UnauthorizedException("You are unauthorized");
		}
		...
		mapSqlParameterSource.addValue("branchId", branchId);
		return jdbc.query(
				"SELECT ext.id as expenseTypeId,ext.name as expenseTypeName,ext.name_kh as expenseTypeKh,"
				...
				mapSqlParameterSource, (rs, rowNum) -> new ExpenseTypeSummaryDetail(rs.getInt("expenseTypeId"),
						rs.getString("expenseTypeName"), rs.getString("expenseTypeKh"), rs.getDouble("expenseAmt"),rs.getInt("expenseItem")));
	}

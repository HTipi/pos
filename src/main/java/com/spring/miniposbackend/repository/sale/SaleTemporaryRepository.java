...
public interface SaleTemporaryRepository extends JpaRepository<SaleTemporary, Long> {
	...
	@Query(value = "select s from SaleTemporary s where s.seat.id=?1 and s.parentSaleTemporary is null and s.id in (?2)") // update
	List<SaleTemporary> findBySeatForPrintByItems(@Param("seatId") Integer seatId,@Param("saleTempIds") List<Long> saleTempIds);
	...
	@Query(value = "select case when count(s)> 0 then true else false end from SaleTemporary s where s.seat.id=?1 and s.parentSaleTemporary is null and s.invoice is null") // update
...
//	List<SaleTemporary> findByUserId(Integer userId, Integer seatId);
	...
	@Modifying
	@Query(value = "update Sales_temp set invoice_id=?1 where user_id=?2 and invoice_id is null and seat_id is null", nativeQuery = true)
	void updateInvoiceByUserId(Long invoiceId, Integer userId);
	
	@Modifying
	@Query(value = "update Sales_temp set invoice_id=?1,customer_id=?3 where user_id=?2 and invoice_id is null and seat_id is null", nativeQuery = true)

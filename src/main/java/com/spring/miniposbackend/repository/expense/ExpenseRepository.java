...
public interface ExpenseRepository extends JpaRepository<Expense, Integer>{
	...
	List<Expense> findByBranchIdWithDate(Integer branchId,@DateTimeFormat(pattern = "yyyy-MM-dd") Date startDate,@DateTimeFormat(pattern = "yyyy-MM-dd") Date endDate, boolean isReversed);

	@Query("select bc from Expense bc where bc.branch.id= ?1 and date_trunc('day',bc.valueDate) between ?2 and ?3 and bc.reverse=false")

...
public class PersonService {
	...
	@Transactional
	  public Person create(PersonRequest request) throws Exception {
	    try {
	      ...
	      if(pers.isPresent())throw new BadRequestException("this person that use this phone number already exit can create account only");
	...
	public PersonNormalUpdateView update(PersonNormalUpdateView personview) throws Exception {
		...
		Sex sex = sexrepository.findById(personview.getSexId()).orElseThrow(() -> new ResourceNotFoundException("There is only male and female"));
	...
	public PersonDetailView uploadimage(MultipartFile file) throws Throwable {
		...
		String baseLocation = imagePathProfile;	
			String fileName = imageUtil.uploadImage(
					baseLocation, person.getId().toString(),
					file);
	...
	public Person updatePrimaryPhone(PersonPrimaryPhoneView personPrimaryphoneView) throws Exception {
		...
		for(int i=0;i<allprimaryphoneindb.size();i++) {
			if(allprimaryphoneindb.get(i).getPrimaryPhone().equalsIgnoreCase(personPrimaryphoneView.getNewprimaryPhone())){
		...
		if(user==null)  new ResourceNotFoundException("user not found");
	...
	public Object checkPrimaryphone(String primaryphone){
		...
		boolean primaryphones = accountRepository.existsByPhone(primaryphone,userProfile.getProfile().getBranch().getId());
		if(person.isPresent()) {
			if(primaryphones) {
